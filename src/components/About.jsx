export default function About({ onRegister }) {
  return (
    <div className="bg-neutral-950">
      <div className="container mx-auto px-4 py-14 md:py-20 grid gap-10 md:grid-cols-2 items-start">
        <div className="space-y-6">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-white">New‑Age Hiring Module</h2>
          <p className="text-neutral-300">
            An invite‑only, in‑person hiring event for tech talent in Hyderabad. Crafted by senior engineers and IIIT‑H alumni with experience at Google, Apple, Microsoft, and Indeed.
          </p>
          <p className="text-neutral-300">
            Forget resume gates. Learn a core concept from experts, take a challenge right after, and get mapped to hiring partners based on performance.
          </p>
          <ul className="grid gap-3 text-neutral-200">
            <li className="flex gap-2"><span>•</span> Learn a high‑impact DSA topic, hands‑on</li>
            <li className="flex gap-2"><span>•</span> Attempt a curated contest immediately</li>
            <li className="flex gap-2"><span>•</span> Get matched to companies purely on skill</li>
            <li className="flex gap-2"><span>•</span> 1:1 mentorship and actionable feedback</li>
          </ul>
          <div className="pt-1">
            <a href="#partners" className="inline-flex items-center rounded-lg border border-white/15 bg-white/0 px-5 py-2.5 text-sm font-medium text-neutral-100 hover:bg-white/5 transition">
              Explore hiring partners
            </a>
          </div>
        </div>

        <div className="rounded-2xl border border-white/10 bg-gradient-to-b from-white/[0.04] to-transparent p-6">
          <h3 className="text-lg font-semibold mb-3 text-white">What you get</h3>
          <div className="grid gap-3 text-sm text-neutral-300">
            <div className="flex gap-3"><span>✓</span> 50+ hiring partners across product & high‑growth startups</div>
            <div className="flex gap-3"><span>✓</span> Pure skill‑based selection — no resume filters</div>
            <div className="flex gap-3"><span>✓</span> 1:1 mentorship from top‑tier engineers</div>
            <div className="flex gap-3"><span>✓</span> Continued support and referrals</div>
          </div>
          <div className="mt-5">
            <button onClick={onRegister} className="inline-flex items-center rounded-lg bg-white text-neutral-900 px-5 py-2.5 text-sm font-medium hover:bg-neutral-200 active:scale-[.99] transition">
              Secure your spot
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
